/**	This library allows the following:
	...
	In the following example i'll use [x/oak]{@link https://deno.land/x/oak} to create HTTP server in Deno:
	...
	- [Nginx → Deno]{@link ./examples/Nginx%20→%20Deno}: how to set up Nginx HTTP server.
	- [Apache → Deno]{@link ./examples/Apache%20→%20Deno}: how to set up Apache HTTP server.
	- [Deno → PHP]{@link ./examples/Deno%20→%20PHP}: how to set up PHP-FPM and forward requests to it.
	- [Deno → spawn-fcgi + Perl]{@link ./examples/Deno%20→%20spawn-fcgi%20%2B%20Perl}: how to set up Perl FastCGI and make requests to it.
	...
	This object is called {@link fcgi}.
	...
	and return without calling `req.respond()`, so other handlers (registered with {@link Fcgi.listen()}) will take chance to handle this request. If none handled, 404 response will be returned.
	...
	{@linkcode Fcgi.options}

	Allows to modify {@link Server} and/or `Client` options. Not specified options will retain their previous values.
	...
	{@linkcode Fcgi.fetchCapabilities}
	...
	{@linkcode Fcgi.canFetch}

	When number of ongoing requests is more than the configured value ([maxConns]{@link ClientOptions.maxConns}), [fetch()]{@link Fcgi.fetch} and [fetchCapabilities()]{@link Fcgi.fetchCapabilities} will wait.
	...
	{@linkcode Fcgi.waitCanFetch}

	### closeIdle()

	{@linkcode Fcgi.closeIdle}

	If `keepAliveTimeout` option was > 0, {@link Fcgi.fetch()} will reuse connections. After each fetch, connection will wait for specified number of milliseconds for next fetch. Idle connections don't let Deno application from exiting naturally.
	You can call `fcgi.closeIdle()` to close all idle connections.

	## Using low-level API

	The mentioned {@link fcgi} object is just a wrapper around low-level functions and classes. It's possible to use them directly.
	...
	Callback given to {@link Fcgi.listen()} receives incoming request as {@link ServerRequest} object. Also asynchronous iteration over {@link Server} yields such objects. {@link ServerRequest} contains information sent from FastCGI server.
	...
	- `body` (Deno.Reader) - Allows to read raw POST body if `req.post.parse()` was not called. The body can be also read from {@link ServerRequest} object itself, as it implements `Deno.Reader` (`req.body == req`).
	...
	- `headersSent` (boolean) - Indicates that response headers are already sent. They will be sent by `respond()` or earlier if you write data to the {@link ServerRequest} object (it implements `Deno.Writer`).
	...
	If using {@link Server} object, it's your responsibility to call `respond()` when you're finished with this request. {@link Fcgi.listen()} API will call `respond()` automatically with 404 status, if you don't call it in any of registered request handlers.

	Response headers and data can be set before calling `respond()`, or they can be given to the `response()`.
	Response body can be given to `respond()`, or it can be written to {@link ServerRequest} object.
	...
	@module
	@summary fcgi - FastCGI protocol implementation for Deno
 **/
